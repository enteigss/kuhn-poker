...
public class KuhnTrainer {
    ...
    // Node representing an information state
    public class Node {
        String infoSet;
        double[] regretSum = new double[NUM_ACTIONS], // regret for regret matching
    ...
    // performs CFR and returns utility for one iteration
    private double cfr(int[] cards, String history, double p0, double p1) {
        ...
        for (int a = 0; a < NUM_ACTIONS; a++) {
            ...
            node.strategySum[a] += (player == 0 ? p0 : p1) * strategy[a];
        }
        return nodeUtil;
    }
